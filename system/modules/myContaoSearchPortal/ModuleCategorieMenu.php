...
class ModuleCategorieMenu extends Module
{
	...
	protected function compile()
	{
		...
		elseif($this->Input->cookie('categories'))
		{
		...
		//Detail-Aufruf auf aktuelle Category setzen
		if($this->Input->get('anzeige'))
		{
		     $adObj = $this->Database->prepare("SELECT 
					     `tl_mcsp_categories`.`pid`,
					     ...
					     WHERE `tl_mcsp_smallads`.`alias`=?")
		                             ->limit(1)
		     			     ->execute($this->Input->get('anzeige'));
		   
		   $this->setCookie('categories', $adObj->pid, $cookieLifeTime); 
		   $actCategory = $adObj->pid;
		   
		   $this->setCookie('subcategories',$adObj->catalias,$cookieLifeTime);		   
		   $subcat = $adObj->catalias;		    		   
