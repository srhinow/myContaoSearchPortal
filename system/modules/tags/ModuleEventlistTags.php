...
class ModuleEventlistTags extends ModuleEventlist
{
	...
	protected function getAllEvents($arrCalendars, $intStart, $intEnd)
	{
		...
		if (strlen($this->Input->get('tag')) || strlen($this->tag_filter))
		{
			$limit = null;
			$offset = 0;
			$tagids = array();
			...
			$tagArray = (strlen($this->Input->get('tag'))) ? array($this->Input->get('tag')) : array();
	...
	protected function compile()
	{
		parent::compile();
		$headlinetags = array();
		if ((strlen($this->Input->get('tag')) && (!$this->tag_ignore)) || (strlen($this->tag_filter)))
		{
