...
class ModuleTagCloud extends Module
{
	...
	{
		if ($this->Input->post('toggleTagCloud') == 1)
		{
			$ts = deserialize($this->Input->cookie('tagcloud_states'), true);
			$ts[$this->Input->post('cloudPageID')][$this->Input->post('cloudID')] = $this->Input->post('display');
	...
	protected function showTags()
	{
		...
		foreach ($this->arrTags as $idx => $tag)
		{
			if (count($pageArr))
			{
				$strUrl = ampersand($this->generateFrontendUrl($pageArr, '/tag/' . str_replace(' ', '+', $tag['tag_name'])));
				if (strlen($strParams))
				{
			...
			if ($tag['tag_name'] == $this->Input->get('tag'))
			{
		...
		foreach ($this->arrRelated as $idx => $tag)
		{
			if (count($pageArr))
			{
				$strUrl = ampersand($this->generateFrontendUrl($pageArr, '/tag/' . str_replace(' ', '+', $this->Input->get('tag')) . '/related/' . str_replace(' ', '+', join(array_merge($relatedlist, array($tag['tag_name'])), ','))));
			}
		...
		$this->Template->selectedtags = (strlen($this->Input->get('tag'))) ? (count($this->arrRelated)+1) : 0;
		...
		$GLOBALS['TL_JAVASCRIPT'][] = 'system/modules/tags/html/js/tagcloud.js';
		if ($this->tag_jumpTo)
		{
